    ...
    <main>
      ...
      <div class="background-box" style="margin: auto; text-align: center; margin-bottom: 60px; margin-top: 100px;">
        <form  method = "post" enctype="multipart/form-data">
          ...
          <p>
            <label for="required-input" style="font-size:20px;">&emsp;&emsp;&emsp;密碼</label>
            <input type="password" name = "password" id="password" required>&emsp;
          </p>
          <div style="text-align:center;">
            <button class="btn btn-primary" style="width: 100px; height: 40px; border-radius: 20px; border: none; border-bottom: 4px solid #0f1730;background: #4d74a7; font-size: 16px; font-weight: 400; color: #fff;">登入</button> &nbsp;&nbsp;
            <a class="btn btn-primary" href="register.html" style="width: 120px; height: 40px; border-radius: 20px; border: none; border-bottom: 4px solid #0f1730;background: #4d74a7; font-size: 16px; font-weight: 400; color: #fff;" >註冊會員</a>
          </div>
          </br>
          <div style="text-align:center;">
            <a href='forgetPwd.html' style="color:rgb(83, 45, 223)">忘記密碼?</a>
          </div>
          </br>
        </form>
      </div>
      <?php
        ...
        if(isset($_POST["email"])){
          if($_POST["email"]!=null){
              ...
              $SQL="SELECT * FROM user WHERE u_email='$email' AND u_pwd='$password' AND u_role='user'";
              //echo $username."</br>";
              //echo $password;
              if($result=mysqli_query($link, $SQL)){
                  if($row=mysqli_fetch_assoc($result))
                  {
                      $username = substr(serialize($row['u_name']),4, strlen(serialize($row['u_name'])));
                      $_SESSION["login"]="Yes";
              ...
              $SQL="SELECT * FROM user WHERE u_email='$email' AND u_pwd='$password' AND u_role='admin'";
